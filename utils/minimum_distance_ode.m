function [dz_opt, dy_opt, out] = ...
    ...
    J0c = find([A1; A2] < -EPS);
    % J0  = setdiff((1:C1.nr+C2.nr)', J0c);
    J1  = setdiff(find(y_opt > EPS), J0c);
    J2e = setdiff((1:C1.nr+C2.nr)', union(J0c, J1));
    % J1c = union(J0c, J2e);

    dx = [dx1; dx2];
    A = [A1; A2];
    dAdx = [dAdx1, zeros(C1.nr, C2.nx);
        zeros(C2.nr, C1.nx), dAdx2];
    ...
    d2Ldxz = [d2Adxz_y1, zeros(nz, C2.nx);
        zeros(nz, C1.nx), d2Adxz_y2];
    ...
    % Case 2: J2e is non-empty.
        ...
        out.J2e = 1;
        P_qp = sparse(d2Ldzz);
        q_qp = d2Ldxz * dx + ALPHA * (dLdz - dAdz(J0c, :)' * y_opt(J0c));
        A_qp = sparse([dAdz(J1, :); dAdz(J2e, :)]);
        l_qp = [-dAdx(J1, :) * dx - ALPHA * A(J1); -Inf * ones(length(J2e), 1)];
        u_qp = [l_qp(1:length(J1)); -dAdx(J2e, :) * dx];
